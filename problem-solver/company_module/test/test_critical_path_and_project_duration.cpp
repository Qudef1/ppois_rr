...
// Тест 6: Проверка, что все задачи критического пути имеют slack=0
TEST_F(CriticalPathAndDurationTest, CriticalPathTasksHaveZeroSlack)
{
  std::string csvData =
      "A;5;\n"
      "B;3;A\n"
  ...
  std::vector<std::string> criticalPathTasks = GetTasksFromTuple(criticalPathTuple);
  ASSERT_GT(criticalPathTasks.size(), 0u) << "Critical path should contain at least one task";
  
  // Получаем все задачи через топологический порядок (более надежный способ)
  ...
  // Строим карту задач по main_idtf
  std::map<std::string, ScAddr> allTasks;
  if (topologicalTuple.IsValid())
  {
  ...
  // Проверяем, что все задачи в критическом пути имеют slack=0
  for (const std::string& taskName : criticalPathTasks)
  {
    if (!allTasks.count(taskName))
    {
      // Пропускаем, если задача не найдена (может быть проблема с получением задач)
      continue;
    }
    ...
    EXPECT_EQ(slack, 0u) << "Task " << taskName << " in critical path should have slack=0";
    
    // Проверяем, что задача помечена как критическая
    bool isCritical = m_ctx->CheckConnector(
      ProjectSchedulingKeynodes::concept_crytical_task,
      task,
      ScType::ConstPermPosArc);
    EXPECT_TRUE(isCritical) << "Task " << taskName << " should be marked as critical";
  }
}

// Тест 7: Проверка сохранения продолжительности для пустого проекта (должен быть 0 или ошибка)
TEST_F(CriticalPathAndDurationTest, ProjectDurationForEmptyProject)
{
  // Проект с одной задачей
...
// Тест 8: Проверка корректности критического пути через сравнение с вычисленной длительностью
